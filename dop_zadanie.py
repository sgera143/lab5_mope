...
def main(m, n,setk):
    ...
    for i in range(len(t)):
        if get_value(t_table, f_3) < t[i]:
            k2[i] +=1
        if get_value(t_table, f_3) > t[i]:
            k[i] += 1  # Сумування кількості чисел t
            beta_i[i] = 0
            d -= 1
    if n == d:
        n = 8 if n == 4 else 15
    ...
    if f_p > get_value(f_t, f_3)[f_4]:
        ...
        if setk==2:
            tWord = ["b0", "b1", "b2", "b3", "b12", "b13", "b23", "b123", "b11", "b22", "b33"]
            for i in range(11):
                print("за", k3, " разів  коефіціент", tWord[i], " був значимим ", k2[i])
                print("за ", k3, "разів  коефіціент", tWord[i], " був незначимим ", k[i])

for i in range(100):
    if  i==0:
        main (m=3,n=15,setk=0)
    else:
        if i !=99:
            main(m=3, n=15,setk=1)
    if i==99:
        main(m=3,n=15,setk=2)
